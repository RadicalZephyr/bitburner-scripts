 ...
 */
export function isStructuralEqual(a, b) {
 ...
 */
export function isArrayStructuralEqual(a, b) {
    if (a.length !== b.length)
        return false;
    for (let i = 0; i < a.length && i < b.length; i++) {
        if (!isStructuralEqual(a[i], b[i]))
            return false;
    }
    return true;
}
 ...
 */
export function isObjectStructuralEqual(a, b) {
 ...
 */
export function isMapStructuralEqual(a, b) {
    if (a.size !== b.size)
        return false;
    for (const k of a.keys()) {
        if (!isStructuralEqual(a.get(k), b.get(k)))
            return false;
    }
    return true;
}
