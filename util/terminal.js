 ...
 */
export function sendTerminalCommand(ns, command, options = {}) {
    const o = {
        ...DEFAULT_OPTIONS,
        ...options,
    };
    ...
    for (const c of sequenceOfCommands) {
        promises.push(withTerminalLock(() => sendOneTimedTerminalCommand(ns, c, o)));
    }
 ...
 */
export function splitAtTimedCommands(commands) {
    const finalCommands = [];
...
function isTimedCommand(command) {
    const TIMED_COMMANDS = /^(analyze|backdoor|grow|hack|weaken)\b/i;
    return TIMED_COMMANDS.test(command);
}
export function tokenize(commands) {
    return commands
        .split(';')
        .map((c) => c.trim())
        .filter((s) => s.length !== 0);
}
let terminalLock = Promise.resolve();
/**
 * Chains promises so only one terminal command runs at a time.
 */
function withTerminalLock(fn) {
...
async function sendOneTimedTerminalCommand(ns, command, opts) {
 ...
 */
export async function findTerminalInput(ns, timeoutMs = DEFAULT_OPTIONS.commandTimeoutMs) {
    let termInputEl;
    const start = Date.now();
    while (true) {
        termInputEl = globalThis['document'].getElementById('terminal-input');
        if (termInputEl)
            break;
        if (timeoutMs > 0 && Date.now() > start + timeoutMs)
            throw new Error(`Timed out after ${timeoutMs}ms waiting for terminal input`);
        await ns.asleep(100);
    }
 ...
 */
function dispatchReactInputAndEnter(terminalInput, command) {
...
function expectedMillisFor(ns, currentServer, cmd) {
...
function getHostFromPrompt(terminalInput) {
    const promptEl = assertEl(terminalInput.previousElementSibling, 'Could not find terminal prompt element.');
