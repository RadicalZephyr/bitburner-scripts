...
function checkWorkers(ns, allocations, workers) {
    ...
    for (const w of workers) {
        ...
        if (Math.abs(actualInUse - used) > 0.0001) {
            ns.print(`WARN: worker ${w.hostname} is not using all allocated RAM  `
                + `snapshot ${ns.formatRam(used)} actual ${ns.formatRam(actualInUse)}`);
            for (const alloc of allocations) {
                if (ns.isRunning(alloc.pid)
                ...
                if (allocClaims.length > 0) {
                    const claims = allocClaims
                        ...
                        .join('');
                    ns.print(`INFO: allocating process ${alloc.pid} running ${alloc.filename}`
                        + `\n  claims: ${claims}`);
                }
                else {
        ...
        if (used > w.totalRam + 0.0001) {
            ns.print(`ERROR: worker ${w.hostname} uses ${ns.formatRam(used)} `
                + `of ${ns.formatRam(w.totalRam)}`);
        }
    }
}
function checkAllocations(ns, allocations) {
    for (const alloc of allocations) {
        ...
        for (const host of alloc.hosts) {
            const claimedChunks = alloc.claims
                .filter((c) => c.hostname === host.hostname
                && c.chunkSize === host.chunkSize)
                .reduce((sum, c) => sum + c.numChunks, 0);
            if (claimedChunks > host.numChunks) {
                ns.print(`ERROR: allocation ${alloc.allocationId} on ${host.hostname} `
...
function crossCheck(ns, snapshot) {
    for (const worker of snapshot.workers) {
        ...
        if (Math.abs(total - usedRam) > 0.001) {
            ns.print(`ERROR: worker ${worker.hostname} reports ${ns.formatRam(usedRam)} `
