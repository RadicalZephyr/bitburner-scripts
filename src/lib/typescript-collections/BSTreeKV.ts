import * as util from 'lib/typescript-collections/util';
import Queue from 'lib/typescript-collections/Queue';
...
export default class BSTreeKV<K, V extends K> {
    ...
    private heightAux(node: BSTreeNode<V> | null): number {
        ...
        return (
            Math.max(this.heightAux(node.leftCh), this.heightAux(node.rightCh))
            + 1
        );
    }
