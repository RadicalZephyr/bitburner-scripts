export interface RingBufferOptions {
    /**
     * If true (default), capacity is rounded up to the next power of two,
     * allowing fast wraparound via bitmasking instead of modulo.
     */
    roundCapacityToPow2?: boolean;

    /**
     * How to clear references on clear()/dequeue:
     * - "slot": overwrite only the slots that are removed (lowest churn, default)
     * - "buffer": allocate a new backing array on clear() (fast mass release of refs)
     */
    clearStrategy?: 'slot' | 'buffer';
}
...
export class RingBuffer<T> {
    ...
    private head = 0; // index of next item to read
    private tail = 0; // index after last written item
    private _size = 0;

    private readonly cap: number; // backing array length (possibly rounded)
    private readonly mask: number; // cap - 1 if pow2; else -1
    private readonly useMask: boolean;
    private readonly clearStrategy: 'slot' | 'buffer';

    constructor(capacity: number, opts: RingBufferOptions = {}) {
        ...
        this.store = new Array<T | undefined>(this.cap);
    ...
    shift(): T | undefined {
        ...
        if (this.clearStrategy === 'slot') this.store[idx] = undefined; // drop ref
