...
`);
    ...
    const algos: [
        (
            ...
            t: string,
            m: number,
        ) => { n: number; growThreads: number; weakenThreads: number },
        string,
    ][] = [
...
function calculateSowThreadsForMaxThreads1(
    ns: NS,
    target: string,
    maxThreads: number,
) {
    let n = 0;
    let { growThreads, weakenThreads } = calculateSowBatchThreads(
        ns,
        target,
        maxThreads,
    );
...
function calculateSowThreadsForMaxThreads2(
    ns: NS,
    target: string,
    maxThreads: number,
) {
...
function calculateSowThreadsForMaxThreads3(
    ns: NS,
    target: string,
    maxThreads: number,
) {
    ...
    for (let i = 0; i < 16; i++) {
        ...
        const { growThreads, weakenThreads } = calculateSowBatchThreads(
            ns,
            target,
            mid,
        );
