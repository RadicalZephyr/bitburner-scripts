...
function growMultiplierForThreads(
  ns: NS,
  target: string,
  threads: number,
  applyMults: boolean,
...
function predictGrowFormula(
  ...
  const moneyAfter =
    ns.formulas.hacking.growAmount(server, player, threads) * mult;
...
`);
  ...
  for (let threads = 1; threads <= maxThreads; threads++) {
    for (let i = 0; i < iterations; i++) {
      ...
      const predHF = useFormulas
        ? predictHackFormula(ns, target, threads, false)
        : { money: NaN, sec: NaN };
      const predHBIM = predictHackBuiltIn(ns, target, threads, true);
      const predHFM = useFormulas
        ? predictHackFormula(ns, target, threads, true)
        : { money: NaN, sec: NaN };
...
async function testGrow(
  ...
  const predGF = useFormulas
    ? predictGrowFormula(ns, target, threads, false)
    : { money: NaN, sec: NaN };
  const predGBIM = predictGrowBuiltIn(ns, target, threads, true);
  const predGFM = useFormulas
    ? predictGrowFormula(ns, target, threads, true)
    : { money: NaN, sec: NaN };
