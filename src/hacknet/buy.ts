...
function upgradeLevelCandidate(
    ns: NS,
    index: number,
    baseGain: number,
): UpgradeCandidate {
    ...
    switch (candidate.type) {
        case 'node': {
            ...
            if (index !== -1) {
                ...
                ns.print(
                    `SUCCESS: purchased ${upgradeDescription(ns, candidate)}`,
                );
            } else {
