...
export function getLimitedRawProduction(
    ...
    const employeeMult =
        (Math.pow(ops, 0.4) + Math.pow(eng, 0.3)) * managementFactor;
...
function buyInputs(
    ...
    const amounts: Record<CorpMaterialName, number> = {} as Record<
        CorpMaterialName,
        number
    >;
    ...
    for (const [name, coeff] of Object.entries(requiredMaterials)) {
        ...
        amounts[name as CorpMaterialName] = Math.max(
            required - material.stored,
            0,
        );
    }
    ...
    if (minUnits < totalRawProduction) {
        for (const name of Object.keys(amounts)) {
            amounts[name as CorpMaterialName] = Math.max(
                minUnits * requiredMaterials[name as CorpMaterialName]
                    - corp.getMaterial(division, city, name).stored,
                0,
            );
        }
    }
