  ...
  GANG_jobCheckInterval  Delay between evaluations`);
    ...
    while (true) {
        ...
        if (
            ...
            info.wantedLevel < CONFIG.minWantedLevel
            && info.wantedLevelGainRate < 0
        ) {
            numHeating++;
        }
...
function splitMembers(
    ns: NS,
    memberNames: string[],
): [GangMemberInfo | null, GangMemberInfo[], GangMemberInfo[]] {
    const isHackingGang = ns.gang.getGangInformation().isHacking;
    ...
    const numTrain = Math.max(
        0,
        Math.floor(members.length * CONFIG.trainingPercent),
    );
