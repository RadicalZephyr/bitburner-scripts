...
export function distributeTasks(
    ...
    const respectDeficit = Math.max(
        0,
        info.respectForNextRecruit - info.respect,
    );
    const horizon = info.respectGainRate * CONFIG.recruitHorizon;
    const respectFraction =
        horizon <= 0 ? 0 : Math.min(1, respectDeficit / horizon);

    const coolingFraction =
        info.wantedPenalty > CONFIG.maxWantedPenalty
            ? Math.min(
                  1,
                  (info.wantedPenalty - CONFIG.maxWantedPenalty)
    ...
    const numRespect = Math.round(
        (total - numCooling - numWar) * respectFraction,
    );
    ...
    coolingNames.forEach(
        (n) => coolingTask && ns.gang.setMemberTask(n, coolingTask),
    );
    idx += numCooling;
    ...
    respectNames.forEach(
        (n) => respectTask && ns.gang.setMemberTask(n, respectTask),
    );
    idx += numRespect;
