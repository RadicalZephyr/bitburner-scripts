...
export class TaskAnalyzer {
    ...
    refresh() {
        ...
        for (const task of this.tasks) {
            money.set(
                task,
                calculateMoneyGain(this.ns, gangInfo, avgMember, task),
            );
            respect.set(
                task,
            ...
            wanted.set(
                task,
                calculateWantedGain(this.ns, gangInfo, avgMember, task),
            );
        }
    ...
    private computeRoleProfiles() {
        const groups: Record<Role, GangTaskStats[]> = {
            ...
            respectGrind: this.tasks.filter(
                (t) =>
                    t.baseRespect > t.baseMoney
                    && t.baseRespect > 0
                    && t.baseWanted >= 0,
            ),
            moneyGrind: this.tasks.filter(
                (t) =>
                    t.baseMoney >= t.baseRespect
                    && t.baseMoney > 0
                    && t.baseWanted >= 0,
            ),
        };
