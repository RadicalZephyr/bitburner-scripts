...
export async function main(ns: NS) {
  ...
  if (typeof contractDataJSON !== 'string') {
    ns.tprintf(
      '%s contract run with non-string data argument. Must be a JSON string of the contract data.',
      scriptName,
    );
    return;
  }
  ...
  const contractData: bigint = BigInt(
    contractDataJSON.substring(1, contractDataJSON.length - 1),
  );
