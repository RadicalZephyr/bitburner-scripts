    ...
    for (const [idx, phase] of phases.map(
        ...
        while (true) {
            if (retryCount > CONFIG.harvestRetryMax) {
                ns.print(
                    `ERROR: harvest repeatedly failed to exec ${script} on ${hostname}`,
                );
            ...
            const pid = ns.exec(
                script,
                hostname,
                { threads: phase.threads * scaling, temporary: true },
                target,
                phase.start,
                lastArg,
                ALLOC_ID_ARG,
            ...
            if (pid === 0) {
                retryCount += 1;
                ns.print(
                    `WARN: failed to exec ${script} on ${hostname}, trying again with fewer threads`,
                );
