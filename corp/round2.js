...
async function upgradeRound2(ns) {
...
async function upgradeDivision(_ns, divName) {
    const division = await _ns('corporation.getDivision', divName);
    for (const city of division.cities) {
        await upgradeOffice(_ns, division, city);
    }
}
async function upgradeOffice(_ns, division, city) {
    await expandOfficeSizeTo(_ns, division, city, 8);
    await setAllWorkersTo(_ns, division, city, 'Research & Development');
}
async function expandToChem(ns) {
    const dispatchClient = new DispatchClient(ns);
    const _ns = dispatchClient.dispatch.bind(dispatchClient);
    const corpInfo = await _ns('corporation.getCorporation');
    const divisions = await Promise.all(corpInfo.divisions.map((name) => _ns('corporation.getDivision', name)));
    ...
    for (const city of CITIES) {
        ...
        let warehouse = await _ns('corporation.getWarehouse', division.name, city);
        if (!warehouse) {
            await _ns('corporation.purchaseWarehouse', division.name, city);
            warehouse = await _ns('corporation.getWarehouse', division.name, city);
            if (!warehouse) {
                ns.ui.openTail();
                ns.print(`WARN: could not buy warehouse for ${division.name} in ${city}`);
                return;
            }
        }
...
async function expandOfficeSizeTo(_ns, division, city, newOfficeSize) {
    ...
    if (office.size < newOfficeSize) {
        ...
        await _ns('corporation.upgradeOfficeSize', division.name, city, seatsToAdd);
    }
    for (let i = office.numEmployees; i < newOfficeSize; i++) {
        await _ns('corporation.hireEmployee', division.name, city);
    }
}
async function setAllWorkersTo(_ns, division, city, task) {
    const office = await _ns('corporation.getOffice', division.name, city);
    for (const job in office.employeeJobs) {
        if (office.employeeJobs[job] === 0)
            continue;
        await _ns('corporation.setAutoJobAssignment', division.name, city, job, 0);
    }
    await _ns('corporation.setAutoJobAssignment', division.name, city, task, office.numEmployees);
}
