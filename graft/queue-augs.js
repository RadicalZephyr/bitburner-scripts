...
function buildMultipliers(ns, mults, presets) {
    if (mults.length === 0 && presets.length === 0)
        return [...MULTIPLIERS];
    ...
    const isMultKey = (m) => Object.hasOwn(exampleMult, m);
...
async function graftAugments(ns, dryRun, mults) {
    ...
    if (graftableAugs.length === 0) {
        ns.clearLog();
        ns.print(`WARN: no graftable augmentations found with desired multipliers ${mults.join(',')} `);
        return;
    }
    graftableAugs.sort((a, b) => {
        ...
        else if (Math.abs(a.installTime - b.installTime)
    ...
    try {
        ...
        ns.tprint(`ERROR: currently busy with something other than grafting, please cancel any other activities before starting grafting.`);
        return;
    }
    await ns.sleep(0);
    if (ns.singularity.isBusy()) {
        ns.print('WARN: Player is busy with an action, refusing to start grafting while busy.');
        return;
    }
    ...
    for (const aug of graftableAugs) {
        ...
        if (result) {
            ...
            ns.print(`failed to graft ${aug.name} or one of its pre-requisites`);
        }
        await ns.sleep(1000);
    }
}
async function graftAugmentation(ns, ownedAugs, aug) {
    ...
    for (const preReqAug of preReqs) {
        if (ownedAugs.has(preReqAug))
            continue;
        // TODO: (ZEFS 2025-08-17 #263) try to purchase augmentation before grafting it
        // Cannot graft pre-req, signal failure
        if (!graftableAugs.has(preReqAug))
            return false;
        const result = await graftAugmentation(ns, ownedAugs, augment(ns, preReqAug));
        if (!result)
            return false;
    }
    ...
    if (!nowOwned.has(aug.name))
        return false;
    ownedAugs.add(aug.name);
    return true;
}
function augment(ns, name) {
...
function scoredAugment(ns, name, mults) {
 ...
 */
function scoreAug(a, mults) {
    let s = 0;
    for (const k of mults) {
        const v = a[k]; // only present if != 1
        if (typeof v === 'number')
            s += v - 1; // additive improvement over baseline
    }
    return s;
}
