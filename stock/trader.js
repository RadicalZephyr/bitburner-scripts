...
export async function main(ns) {
    ...
    function logDecision(action, sym, price, info) {
        rotateLog();
        const entry = {
            ts: Date.now(),
            action,
    ...
    while (true) {
        ...
        for (const sym of symbols) {
            ...
            if (info.zScore < -threshold &&
                price <= buyThresh &&
                shares < maxPosition) {
                const toBuy = Math.min(maxPosition - shares, ns.stock.getMaxShares(sym));
                if (toBuy > 0) {
