...
async function displayChartsAndStuff(ns, company) {
    ...
    ns.printRaw(React.createElement(StockChart, { ns: ns, stockData: stockData }));
}
/**
 ...
 */
async function fetchStockData(ns, company) {
...
function StockChart({ stockData }) {
    ...
    React.useEffect(() => {
        if (stockData === undefined || !containerRef.current)
            return undefined;
...
function stockPlot(data) {
    return Plot.plot({
        ...
        marks: [
            ...
            Plot.text(d3
                .groupSort(data, (g) => d3.max(g, (d) => d.ts), (d) => d.sym)
                .map((sym) => {
