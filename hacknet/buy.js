...
function calculateCurrentGain(ns) {
    const numNodes = ns.hacknet.numNodes();
    let total = 0;
    for (let i = 0; i < numNodes; i++) {
...
function calculateHashGainRate(level, ramUsed, maxRam, cores, mult = 1) {
...
function hashMoneyGain(ns, level, ram, cores) {
...
function newNodeCandidate(ns, baseGain) {
...
function upgradeRamCandidate(ns, index, baseGain) {
    const stats = ns.hacknet.getNodeStats(index);
...
function upgradeCoreCandidate(ns, index, baseGain) {
    const stats = ns.hacknet.getNodeStats(index);
