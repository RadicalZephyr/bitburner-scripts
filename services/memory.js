...
const toFixed = (val) => BigInt(Math.round(val * 100));
const fromFixed = (val) => Number(val) / 100;
let printLog;
...
class MemoryAllocator {
    ...
    pushWorker(hostname, setAsideRam) {
        ...
        printLog(`INFO: registered worker ${hostname} with ` +
        ...
        for (const [id, allocation] of this.allocations.entries()) {
            ...
            for (const claim of allocation.claims) {
                if (!this.ns.isRunning(claim.pid)) {
                    ...
                    printLog(`INFO: reclaimed allocation ${id} ` +
    ...
    claimAllocation(claim) {
        ...
        if (!chunk) {
            printLog(`WARN: claim request for allocation ${claim.allocationId} on ${claim.hostname} not found`);
            return false;
        }
        ...
        if (claimedSoFar + claim.numChunks > chunk.numChunks) {
            printLog(`WARN: claim for allocation ${claim.allocationId} exceeds reserved chunks`);
            return false;
        }
