...
import { LAUNCH_PORT, LAUNCH_RESPONSE_PORT, MessageType, LaunchProtocol, } from 'services/client/launch';
...
async function startLaunchService(ns) {
    ns.disableLog('sleep');
    ...
    constructor(ns) {
        ...
        const handlers = {
            [MessageType.Launch]: async (payload) => {
                const result = await launch(ns, payload.script, payload.options, ...payload.args);
                if (result == null) {
                    return {
                        ok: false,
 ...
 */
async function launch(ns, script, threadOrOptions, ...args) {
    ...
    const allocation = await client.requestTransferableAllocation(ramOverride ?? scriptRam, totalThreads, allocOptions);
    if (!allocation) {
    ...
    for (const allocationChunk of allocation.allocatedChunks) {
        ...
        if (isNaN(threadsHere) || threadsHere < 0)
            continue;
        ...
        const pid = ns.exec(script, hostname, runOptions, ...args, ALLOC_ID_ARG, allocation.allocationId);
        if (!pid) {
...
function baseRunOptions(opts) {
    return {
        ...
        preventDuplicates: opts.preventDuplicates === null
            ? undefined
            : opts.preventDuplicates,
    };
}
function createRunOptions(threads, options) {
