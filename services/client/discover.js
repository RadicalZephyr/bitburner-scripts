...
import { StreamSink, Transaction, Unit } from 'lib/sodium';
import { CONFIG } from 'services/config';
class HostSource {
    #newHostsSource = new ApiStream();
    #resetStream = new StreamSink();
    newHosts = this.#newHostsSource.stream;
    hosts = resettableAccumulator(Set(), this.newHosts, this.#resetStream, (newHost, hosts) => {
        return hosts.add(newHost);
    });
    reset() {
        this.#resetStream.send(Unit.UNIT);
    }
    registerNewHostsSource(source) {
...
const isSubscription = isObjectLike({
...
export class DiscoveryClient {
    ...
    constructor(ns) {
        this.#ns = ns;
        const unlistenWorkers = WorkerSource.newHosts.listen((worker) => {
            this.notifyWorkerSubscriptions(worker);
        });
        const unlistenTargets = TargetSource.newHosts.listen((target) => {
    ...
    notifyTargetSubscriptions(target) {
    ...
    notifyWorkerSubscriptions(worker) {
...
function notifySubscriptions(ns, subscriptions, host) {
    for (const sub of subscriptions) {
        const hostsToSend = host != null ? [...sub.missedUpdates, host] : sub.missedUpdates;
