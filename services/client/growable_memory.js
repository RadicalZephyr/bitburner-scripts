import { MEMORY_PORT, MemoryClient, TransferableAllocation, MessageType, AllocationChunk, MemoryProtocol, isHostAllocation, } from 'services/client/memory';
...
export class GrowableMemoryClient extends MemoryClient {
     ...
     */
    async requestGrowableAllocation(chunkSize, numChunks, options) {
        ...
        const result = await this.client.sendMessageReceiveResponse(MessageType.GrowableRequest, payload);
        if (!result) {
...
export class GrowableAllocation extends TransferableAllocation {
    ...
    port;
    running = true;
    constructor(ns, allocationId, chunks, port) {
     ...
     */
    async startPolling(shouldMergeChunks = false) {
        while (this.running) {
     ...
     * @param shouldMergeChunks - If true, merges new chunks into existing host entries. If false, appends them as separate entries.
     */
    pollGrowth(shouldMergeChunks = false) {
    ...
    async _release(proc, memPort, portClient) {
        this.running = false;
        const release = {
     ...
     */
    async launch(script, threads, ...args) {
        let totalThreads;
        ...
        for (const chunk of this.allocatedChunks) {
            ...
            if (isNaN(threadsHere) || threadsHere < 1)
                continue;
            const hostname = chunk.hostname;
            this.ns.scp([...dependencies, ...explicitDependencies], hostname, 'home');
            ...
            while (true) {
                ...
                const runOptions = baseRunOptions
                    ...
                    : threadsHere;
                const pid = this.ns.exec(script, chunk.hostname, runOptions, ...execArgs);
                if (pid === 0) {
                    retryCount += 1;
...
function appendChunks(dest, add) {
...
function mergeChunks(dest, add) {
    for (const chunk of add) {
        const existing = dest.find((c) => c.hostname === chunk.hostname
