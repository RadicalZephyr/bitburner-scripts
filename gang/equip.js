...
function buildEquipmentList(options) {
    let equipments = [];
    if (options.A) {
        return [allEquipment, []];
    }
...
export function extractValidEquipments(_args) {
    let args = [..._args];
    let equipments = [];
...
function formatEquipment(name, equipments) {
...
function isSubSet(potentialSubSet, superSet) {
