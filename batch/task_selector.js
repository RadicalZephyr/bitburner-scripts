...
import { TASK_SELECTOR_PORT, TASK_SELECTOR_RESPONSE_PORT, MessageType, Lifecycle, TaskSelectorProtocol, } from 'batch/client/task_selector';
import { calculateBatchLogistics, expectedValueForMemory, maxHackPercentForMemory, availableBatchCount, } from 'batch/expected_value';
...
class Server extends BaseServer {
    constructor(ns, manager, monitor) {
...
class TaskSelector {
    ...
    launchedHarvestTasks = new Map();
    constructor(ns, monitor, launcher) {
        this.ns = ns;
    ...
    pushTarget(target) {
        ...
        if (this.harvestTargets.has(target)
    ...
    estimateSowTime(host, threads) {
        ...
        if (threads <= 0 || !Number.isFinite(threads))
            return 0;
    ...
    checkLaunchedTasks() {
        ...
        for (const launch of this.launchedTasks) {
            if (now - launch.time > CONFIG.heartbeatTimeoutMs
    ...
    async launchPendingTasks(memInfo) {
        this.checkLaunchedTasks();
        if (this.launchedTasks.length > 0)
            return;
        const totalProfit = Array.from(this.launchedHarvestTasks.values()).reduce((acc, t) => acc + t.profit, 0);
        let harvestTasks = [...this.pendingHarvestTargets]
            .filter((h) => canHarvest(this.ns, h))
            .map((h) => {
            const hackPercent = maxHackPercentForMemory(this.ns, h, memInfo);
            const logistics = calculateBatchLogistics(this.ns, h, hackPercent);
            if (hackPercent === 0
                || availableBatchCount(memInfo.chunks, logistics.batchRam)
                    < 1) {
                return null;
            }
            const { profit, expectedValue: value } = expectedValueForMemory(this.ns, h, memInfo, hackPercent);
            if (value <= CONFIG.expectedValueThreshold)
                return null;
            return {
                host: h,
                hackPercent,
        ...
        if (totalProfit > 0) {
            harvestTasks = harvestTasks.filter((t) => t.profit >= totalProfit * CONFIG.harvestGainThreshold);
        }
        ...
        if (fallbackHarvest && memInfo.freeRam > minimalHarvestRam) {
            ...
            const minLog = calculateBatchLogistics(this.ns, fallbackHarvest.host);
            const fits = availableBatchCount(memInfo.chunks, minLog.batchRam) >= 1;
        ...
        if (this.sowTargets.size < CONFIG.maxSowTargets) {
            ...
            if (fallback && memInfo.freeRam > minimalSowRam) {
                ...
                const fits = availableBatchCount(memInfo.chunks, gRam + wRam) >= 1;
                if (!lf || lf.nextAttempt <= Date.now()) {
                    const maxRamPerThread = Math.max(gRam, wRam);
                    const threads = Math.floor(memInfo.freeRam / maxRamPerThread);
                    if (threads > 0 && fits) {
    ...
    async launchTill(host, threads) {
        ...
        const result = await this.launcher.launch('/batch/till.js', {
            threads: 1,
    ...
    async launchSow(host, threads) {
        ...
        const result = await this.launcher.launch('/batch/sow.js', {
            threads: 1,
            alloc: { longRunning: true },
        }, host);
        if (!result || result.pids.length === 0) {
    ...
    async launchHarvest(task, maxRam) {
        ...
        const args = maxRam !== undefined
            ? [host, '--max-ram', maxRam, '--port-id', portId]
            : [host, '--port-id', portId];
        const result = await this.launcher.launch('/batch/harvest.js', {
            threads: 1,
            alloc: { longRunning: true },
        }, ...args);
        if (!result || result.pids.length === 0) {
...
async function tick(memory, manager) {
