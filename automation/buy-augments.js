...
`);
    ...
    let budget = ns.getServerMoneyAvailable('home') * augmentationSpendPercentage;
    const ownedAugs = new Set(sing.getOwnedAugmentations(true));
...
export class Aug {
    ...
    rep;
    baseCost;
    constructor(ns, augName, faction) {
        this.name = augName;
...
async function getUnpurchasedAugmentations(ns, factions) {
    const sing = ns.singularity;
    const ownedAugs = new Set(sing.getOwnedAugmentations(true));
    const augs = new Map();
    for (const f of factions) {
        for (const augName of sing.getAugmentationsFromFaction(f)) {
            if (ownedAugs.has(augName) || augs.has(augName))
                continue;
            ...
            if (factionFavor < favorToDonate && factionRep < aug.rep)
                continue;
...
function purchaseAugmentation(ns, ownedAugs, augMap, aug) {
    const sing = ns.singularity;
    const preReqs = sing.getAugmentationPrereq(aug.name);
    for (const pre of preReqs) {
        if (ownedAugs.has(pre))
            continue;
    ...
    if (factionRep < aug.rep && !buyReputation(ns, aug))
        return false;
 ...
 */
export function buyReputation(ns, aug) {
 ...
 * possible.
 */
export function neededReputationCost(ns, aug) {
    const sing = ns.singularity;
...
async function buyNeuroFluxGovernor(ns, budget) {
    const sing = ns.singularity;
 ...
 */
export function augCost(ns, augName) {
 ...
 */
export function getBestFactionForNFG(ns) {
    const factions = ns
        .getPlayer()
    ...
    if (favorFactions.length >= 1)
        return favorFactions[0].name;
    factions.sort((a, b) => b.rep - a.rep);
    if (factions.length >= 1)
        return factions[0].name;
    return null;
}
function canBuyNFGFrom(ns, factionName) {
