...
async function workForCompanies(ns) {
    const companies = allMegaCorps(ns);
...
function isHireable(player, companyRep, info) {
    if (companyRep < info.requiredReputation)
        return false;
    for (const skill of skillNames) {
        if (player.skills[skill] < info.requiredSkills[skill])
            return false;
    }
    return true;
}
