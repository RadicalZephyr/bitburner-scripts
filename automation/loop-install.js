...
import { Aug, augCost, buyReputation, getBestFactionForNFG, neededReputationCost, } from 'automation/buy-augments';
...
`);
    ...
    travelTo(ns, Volhaven);
    study(ns, zbU, algClass);
    void grindIntelligence(ns).catch((e) => ns.print(`ERROR: Grind intelligence failed: ${String(e)}`));
...
async function grindIntelligence(ns) {
...
function study(ns, uni, course) {
    if (!ns.singularity.universityCourse(uni, course, false))
        throw new Error(`could not study ${course} at ${uni}`);
}
async function untilHackLevel(ns, targetLevel) {
    while (true) {
        const hackLevel = ns.getHackingLevel();
        if (hackLevel >= targetLevel)
            return;
        await ns.sleep(1000);
    }
}
async function eventuallyGetBestFactionForNGF(ns) {
...
async function buyOneNeuroFlux(ns, bestFaction) {
    ...
    while (!purchased) {
        ...
        if (!Number.isFinite(donation))
            throw new Error(`Cannot donate to buy Neuroflux Governor, you need more faction rep!`);
        while (!canAfford(ns, donation))
        ...
        while (!canAfford(ns, cost))
            await ns.asleep(1000);
        purchased = ns.singularity.purchaseAugmentation(neuro.faction, neuro.name);
        await ns.asleep(100);
    }
}
async function buyNeuroFlux(ns, bestFaction) {
    const sing = ns.singularity;
    const nfgName = 'NeuroFlux Governor';
    let cost = augCost(ns, nfgName);
    const moneyTracker = await primedMoneyTracker(ns, CONFIG.moneyTrackerHistoryLen, CONFIG.moneyTrackerCadence);
    while (canBuyWithinMaxTime(ns, moneyTracker, cost)) {
        const factionRep = ns.singularity.getFactionRep(bestFaction);
        const neuro = new Aug(ns, nfgName, bestFaction);
        if (factionRep < neuro.rep)
            buyReputation(ns, neuro);
...
function canBuyWithinMaxTime(ns, moneyTracker, cost) {
    const myMoney = ns.getServerMoneyAvailable('home');
    const moneyToEarn = cost - myMoney;
    if (Number.isFinite(moneyToEarn) && moneyToEarn <= 0)
        return true;
    ...
    ns.print(`time to earn next NeuroFlux Governor level: ${ns.tFormat(timeToEarn * 1000)}`);
