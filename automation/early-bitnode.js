...
async function studyAndCode(ns) {
...
function startSleevesShoplifting(ns) {
...
async function untilCombatStat(ns, stat, targetLevel) {
    while (true) {
        const statLevel = ns.getPlayer().skills[stat];
        if (statLevel >= targetLevel)
            return;
        await ns.asleep(1000);
    }
}
function startSleevesWorkingOut(ns) {
    const numSleeves = ns.sleeve.getNumSleeves();
    for (let i = 0; i < numSleeves; i++) {
        if (i % 2 === 0) {
            ...
            if (!ns.sleeve.setToGymWorkout(i, 'Powerhouse Gym', 'dex'))
                throw new Error('failed to workout dexterity at Powerhouse Gym');
        }
    }
}
...
async function sowAndHackNoodles(ns) {
    // Start minimal services
    const pid = ns.run('/start.js', 1, '--minimal');
    if (pid === 0)
        throw new Error('failed to run start script');
    const client = new LaunchClient(ns);
    const noods = 'n00dles';
    const sowResult = await client.launch('/batch/sow.js', {
        threads: 1,
    ...
    const harvestResult = await client.launch('/batch/harvest.js', {
        threads: 1,
        alloc: { longRunning: true },
    }, noods, '--port-id', 111111);
    if (!harvestResult)
